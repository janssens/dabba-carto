...
//Development Tasks
function devHTML(){
  return src(`${options.paths.src.base}/../template.php`).pipe(dest(options.paths.dist.base));
} 
...
function watchFiles(){
  watch(`${options.paths.src.base}/../template.php`,series(devStyles));
...
function prodStyles(){
  ...
  .pipe(purgecss({
    content: ['template.php','include.html'],
